...
public class Parque_Eolico{
     ...
     * O método inicialmente declara um array do tipo double (PTs) com o mesmo tamanho do array colVel recebido no parametro do método e declara ainda um novo aerogerador sendo este o aerogerador pretendido pelo utilizador a class Aerogerador,
     * em seguida ocorre um ciclo for que irá correr todas as posições do array colVel e utilizar o método setVelocidade da class Aerogerador, para o aerogerador declarado inicialmente, atribuindo no parametro do método a velocidade do vento da posição em concreto para,
     ...
     * @param colVel (Coleção de valores das velocidades em varios instantes/dias).
     * @return coleção dos valores da energia/potência na forma de array.
     */

    public double[] getCalcPTVariosInstantes(int n,double[] colVel){
        double[] PTs=new double[colVel.length];
        Aerogerador aerogerador=lista.get(n-1);
        for(int i=0;i<colVel.length;i++){
            aerogerador.setVelocidade(colVel[i]);
